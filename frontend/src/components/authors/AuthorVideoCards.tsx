...
import type { AuthorProps, VideoListProps, VideoProps, VideoStatistics } from "#/types";
...
const AuthorVideoCards = ({ author, videos }: AuthorVideoCardsProps) => {
  ...
  return (
    <Row xs={1} sm={2} md={3} lg={4} xl={5} xxl={6} className={"px-2 pb-3"}>
      {videos.map((video) => (
        <AuthorVideoCard key={video.id} video={video} statistics={videoStatistics[video.id]} />
      ))}
    </Row>
  );
};
...
const AuthorVideoCard = ({ video, statistics }: AuthorVideoCardProps) => {
  ...
  return (
    <Col className={"g-2"}>
      <Card className={"cursor-pointer h-100 hover-bg-info-subtle"} onClick={handleClick}>
