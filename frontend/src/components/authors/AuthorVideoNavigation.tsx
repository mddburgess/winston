...
const AuthorVideoNavigation = ({ videos, index }: AuthorVideoNavigationProps) => {
  ...
  return (
    <Row className={"d-print-none mb-2 justify-content-between"}>
      <Col>{prevVideo && <VideoNavigation video={prevVideo} direction={"prev"} />}</Col>
      <Col>{nextVideo && <VideoNavigation video={nextVideo} direction={"next"} />}</Col>
    </Row>
  );
};
...
const VideoNavigation = ({ video, direction }: VideoNavigationProps) => {
  ...
  const alignment = direction === "prev" ? "justify-content-start text-start" : "justify-content-end text-end";
  ...
  return (
    <Row className={"border cursor-pointer h-100 hover-bg-info-subtle mx-0 p-2 rounded"} onClick={handleClick}>
      {direction === "prev" && (
