...
export const AppEventsSource = ({
  ...
  useEventSourceListener<SubscriptionEvent>(
    {
      ...
      event: {
        name: "message",
        listener: (event) => {
          console.debug("Subscribed to app events:", event.data.subscriptionId);
  ...
  useEventSourceListener<AppEvent>(handleEvent("pull-operation", onOperationEvent), [eventSource, onOperationEvent]);

  useEventSourceListener<AppEvent>(handleEvent("pull-data", onDataEvent), [eventSource, onDataEvent]);
