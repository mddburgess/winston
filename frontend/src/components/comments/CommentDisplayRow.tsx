...
import { EyeSlash, EyeSlashFill, Flag, FlagFill, ReplyFill } from "react-bootstrap-icons";
  ...
  const rowClass = comment.properties.important
    ...
    : comment.properties.hidden
      ...
      : highlight
        ...
        : "";
  const linkClass = comment.properties.hidden ? "small text-body-tertiary" : "small";
  ...
  return (
    <>
      <Row className={`gx-2 ${rowClass}`}>
        ...
        <Col xs={"auto"}>
          <AuthorProfileImage author={comment.author} minWidth={"1.5rem"} />
        </Col>
        <Col xs={"auto"} className={"flex-center"}>
