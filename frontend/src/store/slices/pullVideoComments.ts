...
const pullVideoCommentsSlice = createSlice({
  ...
  reducers: {
    ...
    updatePullCommentsData: (
      ...
      const newCommentReplyIds = (comments ?? []).flatMap((comment) => comment.replies ?? []).map((reply) => reply.id);
      const newReplyIds = map(replies ?? [], "id");

      const commentIds = uniq([...existingCommentIds, ...newCommentIds]);
      const replyIds = uniq([...existingReplyIds, ...newCommentReplyIds, ...newReplyIds]);
...
const { clearPullComments, requestPullComments, updatePullCommentsData, updatePullCommentsStatus } =
