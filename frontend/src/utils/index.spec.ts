...
describe(ascBy, () => {
  it("sorts values in ascending order by the specified property", () => {
    ...
    expect(list.sort(ascBy((str) => str.length))).toStrictEqual(["pear", "apple", "banana"]);
  });
});
...
describe(descBy, () => {
  it("sorts values in descending order by the specified property", () => {
    ...
    expect(list.sort(descBy((str) => str.length))).toStrictEqual(["banana", "apple", "pear"]);
  });
});
...
describe(sumBy, () => {
  ...
  it("returns the sum of a list of items using a property selector", () => {
    const items: Item[] = [{ property: 1 }, { property: 4 }, { property: 9 }];
