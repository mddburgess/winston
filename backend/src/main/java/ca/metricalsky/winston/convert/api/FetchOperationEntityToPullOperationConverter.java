...
public abstract class FetchOperationEntityToPullOperationConverter
        implements Converter<FetchOperationEntity, PullOperation> {

    @Override
    @Named("convertFetchOperationEntityToPullOperation")
    public PullOperation convert(FetchOperationEntity source) {
        return switch (source.getOperationType()) {
            case CHANNELS -> convertChannel(source);
            case VIDEOS -> convertVideos(source);
            case COMMENTS -> convertComments(source);
            case REPLIES -> convertReplies(source);
        };
    }
